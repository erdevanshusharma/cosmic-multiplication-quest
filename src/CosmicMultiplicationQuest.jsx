...
const CosmicMultiplicationQuest = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      star.style.animation = `pulse ${3 + Math.random() * 4}s infinite ${Math.random() * 5}s`;
  ...
  const [gameState, setGameState] = useState(loadFromLocalStorage("cosmicQuest_gameState", "menu")); // menu, game, metrics
  const [currentPlanet, setCurrentPlanet] = useState(loadFromLocalStorage("cosmicQuest_currentPlanet", 1));
  const [score, setScore] = useState(loadFromLocalStorage("cosmicQuest_score", 0));
  const [lives, setLives] = useState(loadFromLocalStorage("cosmicQuest_lives", 3));
  const [feedback, setFeedback] = useState("");
  const [currentQuestion, setCurrentQuestion] = useState({});
  const [answerOptions, setAnswerOptions] = useState([]);
  const [timerRunning, setTimerRunning] = useState(false);
  const [timeRemaining, setTimeRemaining] = useState(10);
  const [unlockedPlanets, setUnlockedPlanets] = useState(loadFromLocalStorage("cosmicQuest_unlockedPlanets", [1]));
  const [speedBoost, setSpeedBoost] = useState(1);
  const [correctAnswers, setCorrectAnswers] = useState(loadFromLocalStorage("cosmicQuest_correctAnswers", {}));
  const [wrongAnswers, setWrongAnswers] = useState(loadFromLocalStorage("cosmicQuest_wrongAnswers", {}));
  const [spaceshipParts, setSpaceshipParts] = useState(loadFromLocalStorage("cosmicQuest_spaceshipParts", 0));
  const [badges, setBadges] = useState(loadFromLocalStorage("cosmicQuest_badges", []));
  const [miniGameActive, setMiniGameActive] = useState(false);
  const [miniGameType, setMiniGameType] = useState("");
  const [planetMastery, setPlanetMastery] = useState(loadFromLocalStorage("cosmicQuest_planetMastery", {})); // Tracks mastery level for each planet
  const [fastAnswers, setFastAnswers] = useState(loadFromLocalStorage("cosmicQuest_fastAnswers", {})); // Tracks fast answers for each planet
  const [lastPlayed, setLastPlayed] = useState(loadFromLocalStorage("cosmicQuest_lastPlayed", null)); // Last played timestamp
  ...
  useEffect(() => {
    if (lastPlayed && score > 0) {
      ...
      const now = new Date();
      const daysSinceLastPlayed = Math.floor((now - lastPlayedDate) / (1000 * 60 * 60 * 24));
      
      if (daysSinceLastPlayed >= 1) {
        // Only show welcome back message if it's been at least a day
        const welcomeMessage = daysSinceLastPlayed === 1 
  ...
  const planets = [
    { id: 1, name: "Earth", table: 3, color: "bg-blue-400", difficulty: "easy" },
    { id: 2, name: "Mars", table: 4, color: "bg-red-500", difficulty: "easy" },
    { id: 3, name: "Jupiter", table: 5, color: "bg-orange-300", difficulty: "easy" },
    { id: 4, name: "Saturn", table: 6, color: "bg-yellow-600", difficulty: "medium" },
    { id: 5, name: "Uranus", table: 7, color: "bg-blue-300", difficulty: "medium" },
    { id: 6, name: "Neptune", table: 8, color: "bg-blue-600", difficulty: "medium" },
    { id: 7, name: "Pluto", table: 9, color: "bg-purple-400", difficulty: "medium" },
    { id: 8, name: "Kepler-186f", table: 10, color: "bg-green-500", difficulty: "hard" },
    { id: 9, name: "Trappist-1e", table: 11, color: "bg-red-300", difficulty: "hard" },
    { id: 10, name: "Proxima Centauri b", table: 12, color: "bg-pink-400", difficulty: "hard" },
    { id: 11, name: "HD 189733b", table: 13, color: "bg-indigo-400", difficulty: "expert" },
    { id: 12, name: "WASP-12b", table: 14, color: "bg-purple-600", difficulty: "expert" },
    { id: 13, name: "CoRoT-7b", table: 15, color: "bg-amber-700", difficulty: "expert" },
    { id: 14, name: "K2-18b", table: 16, color: "bg-cyan-500", difficulty: "expert" },
    { id: 15, name: "GJ 1214b", table: 17, color: "bg-lime-600", difficulty: "master" },
    { id: 16, name: "TOI-1452b", table: 18, color: "bg-teal-400", difficulty: "master" },
    { id: 17, name: "55 Cancri e", table: 19, color: "bg-rose-500", difficulty: "master" },
    { id: 18, name: "Kepler-442b", table: 20, color: "bg-emerald-600", difficulty: "master" },
  ];
  ...
  const generateQuestion = () => {
    ...
    while (wrongAnswers.length < 3) {
      ...
      if (errorType < 0.3) {
        ...
        const wrongTable = multiplicand + (Math.random() < 0.5 ? tableOffset : -tableOffset);
  ...
  const submitAnswer = (selectedAnswer) => {
    ...
    if (isCorrect) {
      ...
      const difficultyMultiplier = 
        planet.difficulty === "easy" ? 1 :
        planet.difficulty === "medium" ? 2 :
        planet.difficulty === "hard" ? 3 :
        planet.difficulty === "expert" ? 4 : 5; // 5x for master level
      
      const timeBonus = Math.max(1, Math.ceil(timeRemaining / 2));
      const pointsEarned = 10 * difficultyMultiplier * timeBonus * speedBoost;
      ...
      setPlanetMastery(prev => {
        ...
        const newMasteryPercent = Math.floor((correctCount + 1) / totalAttempts * 100);
      ...
      if (currentMastery >= 85 && fastAnswersCount >= 10 && 
          !unlockedPlanets.includes(currentPlanet + 1) && 
          currentPlanet < planets.length) {
        setUnlockedPlanets(prev => [...prev, currentPlanet + 1]);
        setBadges(prev => [...prev, `Unlocked ${planets[currentPlanet].name}`]);
        setFeedback(prev => `${prev} 🎉 You've unlocked ${planets[currentPlanet].name}!`);
      }
      ...
      setPlanetMastery(prev => {
        ...
        const newMasteryPercent = Math.floor(correctCount / totalAttempts * 100);
  ...
  const triggerMiniGame = () => {
    ...
    while (wrongAnswers.length < 3) {
      ...
      if (Math.random() < 0.5) {
        ...
        wrongAnswer = Math.random() < 0.5 ? firstNum + secondNum : firstNum * (secondNum + 1);
      }
  ...
  const completeMiniGame = (selectedAnswer) => {
    ...
    if (success) {
      ...
      setBadges(prev => [...prev, `${miniGameType === "asteroid" ? "Asteroid Dodger" : "Safe Lander"}`]);
    } else {
      setFeedback(`Mini-game failed. The answer was ${currentQuestion.answer}.`);
    }
  ...
  const resetProgress = () => {
    if (window.confirm("Are you sure you want to reset all your progress? This cannot be undone.")) {
      ...
      const keysToRemove = [
        "cosmicQuest_gameState", "cosmicQuest_currentPlanet", "cosmicQuest_score",
        "cosmicQuest_lives", "cosmicQuest_unlockedPlanets", "cosmicQuest_correctAnswers",
        "cosmicQuest_wrongAnswers", "cosmicQuest_spaceshipParts", "cosmicQuest_badges",
        "cosmicQuest_planetMastery", "cosmicQuest_fastAnswers", "cosmicQuest_lastPlayed"
      ];
  ...
  const createNewProfile = () => {
    if (window.confirm("This will create a completely new profile and clear ALL data. Are you sure you want to continue? This cannot be undone.")) {
  ...
  const getMasteryData = () => {
    return planets.map(planet => {
      const tableQuestions = Object.keys(correctAnswers).filter(key => 
        parseInt(key.split('x')[0]) === planet.table
      );
      
      const totalCorrect = tableQuestions.reduce((sum, key) => sum + correctAnswers[key], 0);
      const totalWrong = tableQuestions.reduce((sum, key) => sum + (wrongAnswers[key] || 0), 0);
  ...
  const renderMenu = () => (
    <div className="space-y-8 text-center">
      <div className="mb-8">
        <h1 className="text-5xl font-bold text-white mb-6" style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.7)' }}>
          Cosmic Multiplication Quest
        </h1>
        <p className="text-blue-200 text-xl">Explore the universe while mastering multiplication tables!</p>
      </div>
      
      <div className="relative inline-block mx-auto mb-12" style={{ animation: 'spaceship-idle 3s ease-in-out infinite' }}>
        <div className="w-48 h-48 bg-blue-500 rounded-full mx-auto flex items-center justify-center relative">
          <div className="absolute inset-0 rounded-full opacity-50 bg-purple-500" style={{ animation: 'pulse 2s infinite' }}></div>
      ...
      <div className="mt-12 text-gray-300 max-w-md mx-auto rounded-lg bg-gray-800 bg-opacity-70 p-4">
        ...
        <p className="text-sm">
          Explore 18 unique planets, each representing a multiplication table from 3 to 20. 
          Answer correctly to earn points and unlock new planets! 
          Master the advanced tables to become a true Space Mathematician!
          Watch out for special mini-games and collect spaceship parts along the way!
        </p>
      </div>
      ...
      <div className="mt-6 flex justify-center space-x-4">
        ...
        {(score > 0 || unlockedPlanets.length > 1 || Object.keys(correctAnswers).length > 0) && (
          <button 
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-6 bg-gray-800 bg-opacity-70 rounded-lg p-3 shadow-lg">
        <div className="text-white">
          <div className="flex items-center">
            <div className={`w-6 h-6 ${planets.find(p => p.id === currentPlanet).color} rounded-full mr-2`}></div>
            <p className="font-bold">{planets.find(p => p.id === currentPlanet).name}</p>
          </div>
          <p className="text-blue-300">Table: {planets.find(p => p.id === currentPlanet).table}'s</p>
        </div>
        <div className="text-white">
          <p className="text-yellow-300 font-bold">Score: {score}</p>
          <p>Lives: {Array(lives).fill("❤️").join("")}</p>
          {speedBoost > 1 && (
            <p className="text-xs text-green-300">Speed Boost: x{speedBoost.toFixed(1)}</p>
          )}
        </div>
      </div>
      ...
      {!miniGameActive ? (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
          <div className="mb-4">
            <div className="flex justify-between items-center">
              ...
              <div 
                ...
                style={{ boxShadow: timeRemaining < 5 ? '0 0 10px rgba(239, 68, 68, 0.7)' : 'none' }}
              >
            ...
            <div className="relative">
              ...
              >
                <span className="text-blue-300">{currentQuestion.multiplicand}</span> 
                <span className="text-white"> × </span> 
                <span className="text-purple-300">{currentQuestion.multiplier}</span> 
                <span className="text-white"> = </span>
          ...
          {feedback && (
            <div className={`mt-4 p-3 rounded-lg text-center text-lg font-semibold ${feedback.includes('Correct') ? 'bg-green-700 text-white' : 'bg-red-700 text-white'}`}>
              {feedback}
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg relative overflow-hidden">
          ...
          {miniGameType === "asteroid" ? (
            <>
              ...
              <div className="absolute w-8 h-8 rounded-full bg-gray-500 opacity-30 bottom-20 left-10" style={{ animation: 'pulse 3s infinite 1s' }}></div>
              <div className="absolute w-12 h-12 rounded-full bg-gray-600 opacity-20 top-20 left-20" style={{ animation: 'pulse 4s infinite 2s' }}></div>
            </>
          ) : (
            <>
          ...
          <h2 className="text-3xl text-white mb-4 font-bold relative" style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.5)' }}>
            {miniGameType === "asteroid" ? (
              <span className="text-yellow-300">⚠️ Asteroid Field Alert! ⚠️</span>
            ) : (
              <span className="text-blue-300">🌍 Planetary Landing Approach 🛬</span>
            )}
          </h2>
  ...
  const renderMetrics = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="text-white space-y-2">
              ...
              <p>Planets Unlocked: {unlockedPlanets.length} / {planets.length}</p>
              {lastPlayed && (
              ...
              <div className="mt-4 pt-4 border-t border-gray-700">
                <h3 className="text-lg text-blue-300 mb-2">Progression System:</h3>
                <ul className="text-sm space-y-1 text-gray-300">
                  <li>• Master each planet before unlocking the next one</li>
                  <li>• You need 85% accuracy and 10 fast answers to progress</li>
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Planetary Systems</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-8">
          {planets.map(planet => {
            ...
            return (
              <div 
                key={planet.id}
                className={`relative ${planet.color} rounded-full aspect-square flex flex-col justify-center items-center p-2 text-center cursor-pointer transform transition-all hover:scale-105 planet-hover planet-shadow ${isUnlocked ? "opacity-100" : "opacity-50"}`}
                onClick={() => isUnlocked && selectPlanet(planet.id)}
                style={{ 
                  animation: isUnlocked ? `planet-rotate ${20 + planet.id * 10}s linear infinite` : 'none',
              ...
              >
                ...
                {planetData.totalQuestions > 0 && (
                  <>
                    {/* Mastery percentage */}
                    <div className="absolute bottom-1 right-1 w-8 h-8 bg-gray-900 rounded-full flex items-center justify-center text-xs" style={{ boxShadow: '0 0 5px white' }}>
                      <div className={`${
                        planetData.mastery > 90 ? 'text-purple-400' : 
                        planetData.mastery > 80 ? 'text-green-400' : 
                        planetData.mastery > 50 ? 'text-yellow-400' : 
                        'text-red-400'
                      }`}>
                        {planetData.mastery}%
                      </div>
                    ...
                    <div className="absolute top-1 right-1 text-xs bg-gray-900 text-white px-2 py-1 rounded-full" style={{ boxShadow: '0 0 5px white' }}>
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-8">
          {masteryData.filter(p => p.totalQuestions > 0).map(planet => (
            <div key={planet.id} className="mb-5">
              <div className="flex justify-between text-white mb-1">
                <span>{planet.name} ({planet.table}'s)</span>
                <div className="flex space-x-4">
                  <span className={`${
                    planet.mastery > 90 ? 'text-purple-400' : 
                    planet.mastery > 80 ? 'text-green-400' : 
                    planet.mastery > 50 ? 'text-yellow-400' : 
                    'text-red-400'
                  }`}>
                    Mastery: {planet.mastery}%
                  </span>
                  <span className={`${
                    (fastAnswers[planet.id] || 0) >= 10 ? 'text-green-400' : 'text-gray-400'
                  }`}>
              ...
              <div className="w-full bg-gray-700 rounded-full h-4 mb-1">
                <div 
                  className={`h-4 rounded-full ${
                    planet.mastery > 90 ? 'bg-purple-500' : // Outstanding mastery (new level)
                    planet.mastery > 80 ? 'bg-green-500' : // Strong mastery
                    planet.mastery > 50 ? 'bg-yellow-500' : // Moderate mastery
                    'bg-red-500' // Needs practice
                  }`}
                  style={{ 
                    width: `${planet.mastery}%`, 
                    backgroundImage: planet.mastery > 90 ? 'linear-gradient(to right, #8b5cf6, #d946ef)' : 'none' 
                  }}
                ></div>
              </div>
              ...
              <div className="w-full bg-gray-700 rounded-full h-2">
                <div 
                  ...
                  style={{ 
                    width: `${Math.min(100, (fastAnswers[planet.id] || 0) * 10)}%`
                  }}
                ></div>
              </div>
              ...
              {planet.mastery >= 85 && (fastAnswers[planet.id] || 0) >= 10 ? (
                <div className="text-xs text-green-400 mt-1">Requirements met for next planet!</div>
              ) : planet.mastery >= 85 ? (
                <div className="text-xs text-yellow-400 mt-1">Need more fast answers to progress</div>
              ) : (fastAnswers[planet.id] || 0) >= 10 ? (
                <div className="text-xs text-yellow-400 mt-1">Need higher accuracy to progress</div>
              ) : null}
            </div>
          ))}
          
          {masteryData.filter(p => p.totalQuestions > 0).length === 0 && (
            <p className="text-white">Complete missions to see your progress!</p>
          )}
        </div>

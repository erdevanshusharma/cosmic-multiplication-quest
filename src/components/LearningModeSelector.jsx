...
import { loadLearningModeLevelCompletion, isLearningModeLevelCompleted } from '../utils/LocalStorageUtils';
...
const LearningModeSelector = ({ planet, onSelectLevel, onCancel }) => {
  ...
  return (
    <div className="learning-mode-selector p-6 bg-gray-900 rounded-lg shadow-lg text-white">
      <h2 className="text-2xl font-bold mb-4 text-center">Learning Mode - Table {planet.table}</h2>
      <p className="mb-4 text-gray-300">Select a level to practice specific multiplication ranges</p>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {levels.map(level => {
          const isCompleted = levelCompletionStatus[level.id]?.completed;
          const playerRank = levelCompletionStatus[level.id]?.playerRank || getPlayerRank(null, true);
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                ...
                {isCompleted && (
                  <span className={`px-2 py-1 rounded text-sm ${playerRank.color} bg-gray-700`}>
                    {playerRank.rank}
                  </span>
                )}
              </div>

...
const LearningModeMetrics = ({ 
  ...
  const completionPercentage = Math.min(100, Math.round((totalQuestions / totalQuestionsInRange) * 100));
  ...
  const formattedAvgTime = avgResponseTime !== null ? avgResponseTime.toFixed(1) + 's' : 'N/A';
  
  // We already calculated completion percentage above based on range coverage
  
  // Check if player has achieved Pro rank or better
  const hasAchievedProRank = playerRank && ['Pro', 'Hacker', 'God'].includes(playerRank.rank);

  return (
    <div className="bg-gray-800 rounded-lg p-4 mt-4 shadow-lg">
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-xl text-emerald-400 font-bold">Learning Progress</h3>
        <div className="flex space-x-2">
          ...
          <button 
            ...
            className="px-3 py-1 text-sm bg-purple-600 hover:bg-purple-700 rounded-full text-white"
          >
            Exit Learning
          </button>
        </div>
      ...
      <div className="mb-3">
        <div className="text-gray-300">Table {planet.table} - {currentLearningLevel.description}</div>
        <div className="flex justify-between mt-2">
          <div className="text-white">Facts covered: {totalQuestions}/{totalQuestionsInRange}</div>
      ...
      {hasAchievedProRank && (
        <div className="mt-3 text-center p-2 bg-emerald-800 rounded-lg text-white">
          ✓ Level Mastered! You're ready for the next level!
        </div>
      )}
    </div>
  );
